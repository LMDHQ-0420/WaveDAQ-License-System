...
const release = {
  tag_name: "v1.2.3",
  assets: [
    { id: 101, name: "WaveDAQ-macos-arm64-v1.2.3.dmg", url: `https://api.github.com/repos/${repository}/releases/assets/101`, digest: `sha256:${"a".repeat(64)}` },
    { id: 102, name: "WaveDAQ-macos-x64-v1.2.3.dmg", url: `https://api.github.com/repos/${repository}/releases/assets/102`, digest: `sha256:${"b".repeat(64)}` },
...
test("matches each supported platform and rejects unsupported platforms", () => {
  ...
  assert.equal(platformAssetMatches("WaveDAQ-mac-v1.2.3.dmg", "macos-arm64"), true);
  assert.equal(platformAssetMatches("WaveDAQ-mac-v1.2.3.dmg", "macos-x64"), true);
...
test("uses immutable GitHub asset IDs and correct launch targets", () => {
  ...
  assert.equal(mac.launch_path, "/Applications/WaveDAQ.app");
