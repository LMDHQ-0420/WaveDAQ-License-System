...
type Release = { id: string; product_id: string; version: string; platform: string; sha256: string; status: string };
...
function App() {
  ...
  const [releaseForm, setReleaseForm] = useState({ id: "", product_id: "", version: "", platform: "windows-x64", asset_url: "", sha256: "" });
  ...
  async function createRelease() {
    ...
    setReleaseForm({ ...releaseForm, id: "", version: "", asset_url: "", sha256: "" }); await loadAll();
  }
  ...
  return <main>
    ...
    <Table title="版本" headers={["ID", "产品", "版本", "平台", "状态"]} rows={releases.map((item) => [item.id, item.product_id, item.version, item.platform, item.status])} />
  </main>;
}
